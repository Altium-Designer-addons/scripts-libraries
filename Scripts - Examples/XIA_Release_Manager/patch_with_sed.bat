...
rem ***************************************************************************
rem patch_with_sed.bat
rem    DOS batch file to copy a file to a temp file, run the temp file through
rem a specified sed command, and overwrite original file with output of sed.
rem ***************************************************************************
rem 
rem ***************************************************************************
...
rem *  Check-in Date: $Date: 2011-11-02 17:52:42 -0700 (Wed, 02 Nov 2011) $ 
rem *  Version #:     $Revision: 21006 $
rem *  
...
rem /* Issue cp command. */
%TEMPLATE%\dosutils\UnxUtils\usr\local\wbin\rm.exe -f %OUTFILE%
%TEMPLATE%\dosutils\UnxUtils\usr\local\wbin\cp.exe -p %INFILE% %OUTFILE%

rem /* Issue sed command. */
rem /*  Redirect stdout and stderr from sed.exe to %INFILE% */
rem @ECHO ON
%TEMPLATE%\dosutils\UnxUtils\usr\local\wbin\sed.exe %COMMAND% %OUTFILE% > %INFILE% 2>&1
@ECHO OFF
