{..............................................................................}
{ Fix Overlaps v2.2 Originally Created by:    Petar Perisin                    }
{                                                                              }
{ Summary   This script can be used to clean nets within choosen net class.    }
{                                                                              }
...
{..............................................................................}
{                                                                              }
{ Created Fix Connections v1.0 Randy Clemmons                                  }
{                                                                              }
{ Fix Connections is a modified version of Fix Overlaps v2.2                   }
{                                                                              }
...
{                                                                              }
{ Please note that Petar did all of the heavy lifting to make this script.     }
{ My modifications only added a few nice to have user features.                }
{..............................................................................}
...
// Function returns true if point is on IPCB_track (on line and between start and end point of a track).
...
// Procedure that checks track for bad connections. If track has bad connection, it is selected.
...
begin
   ...
   begin
      ...
      // Progress Meter - Small Performance Hit
      delta := GetTickCount - startTick;
      if delta > Throttle + 50 then  // fresh interval in milliseconds
      begin
           lblTime.Caption := 'Elapsed Time : ' + IntToStr(delta) + ' mSec';
           lblCnt.Update;   // Update Form
           Throttle := delta;
      end;
   ...
   // Progress Meter - Small Performance Hit
   cnt := cnt +1;
   lblCnt.Caption := 'Found Bad Connections : ' + IntToStr(cnt);
   delta := GetTickCount - startTick;
   lblTime.Caption:= 'Elapsed Time : ' + IntToStr(delta) + ' mSec';
   lblCnt.Update;   // Update Form
...
begin
    ...
    Begin
        ...
        // Progress Meter - Added in Fix Connectons v1.0
        delta := GetTickCount - startTick;
        lblTime.Caption := 'Elapsed Time : ' + IntToStr(delta) + ' mSec';
...
begin
   ...
   begin
     // ShowMessage(Net.Name);
      ...
      Net := Iterator.NextPCBObject;
      // I got a bug here - iterator would never break out of this loop in one of
...
// OK, so this is the algorithm for fixing bad connections
...
begin
 ...
 Function GetLog(dummy : String):TStrings;

 // Return string in last line - Added in Fix Connections v1.0
 var
 ...
 begin
   ...
   // Check for user's last selected NetClass
   fileName := ExtractFilePath(PCBServer.GetCurrentPCBBoard.FileName) + 'temp\';
   fileName := fileName + 'NetClass.log';
   //ShowMessage(fileName);

   if FileExists(fileName) then
   begin;
     fileData := TStringList.Create;        // Create TSTringList object
   ...
   begin
     Result := '' // No string data found in file
   end;

 end;

 Procedure PutLog(tStr : String);

 // Save NetClass to File - Added in Fix Connections v1.0
 var
   fileData : TStringList;
   fileName : String;
 ...
 begin
   ...
   // Create a temp folder in the project directory and store the user selected NetClass
   fileName := ExtractFilePath(PCBServer.GetCurrentPCBBoard.FileName) + 'temp\';
   ...
   fileData.SaveToFile(fileName);         // Save the file
...
//   Board.ViewManager_FullUpdate;
   ...
   ComboBoxClasses.Clear;

   // NetClasss Combo default added in Fix Connections v1.0
   // Default Typically will be 'All Nets'
   SetClass := 'All Nets';

   // Get Users Last Choice from NetClass.log File
   SetClass := GetLog('dummy');
   ...
   Begin
       ...
       begin
          ...
          // Set Default to All Nets
          if c.Name = SetClass Then
          begin
               flg := True;
               ComboBoxClasses.ItemIndex := (ComboBoxClasses.Items.IndexOf(c.Name));
          end;
       end;
   ...
   // Default If Desired NetClass is Not Found
   If flg <> True then ComboBoxClasses.ItemIndex := 0;

   FixConnectionsForm.ShowModal;    // Show in Modal Mode
...
procedure TFixConnectionsForm.ButtonUnitsClick(Sender: TObject);
begin
...
procedure TFixConnectionsForm.ButtonCancelClick(Sender: TObject);
begin
   close;
end;

procedure TFixConnectionsForm.ButtonOKClick(Sender: TObject);
var
...
begin
   ...
   // Start Elapsed Timer and Initilaize Bad Connection Counter
   // Added in Fix Connections v1.0
   startTick := GetTickCount;
   lblCnt.Caption := 'Found Bad Connections : ' + IntToStr(cnt);
   lblCnt.Update;   // Update Form

   // Save User Selected NetClass to File - Fix Connections v1.0
   PutLog(ComboBoxClasses.Text);
   ...
   if CheckBoxShortBadConnections.Checked then
      ...
      // Deselect All- added in Fix Connections v1.0
      ResetParameters;
      AddStringParameter('Scope','All');
      RunProcess('PCB:DeSelect');

      // Reset Bad Connections Counter
      cnt := 0;

      // Find and Select Bad Connections in Selected NetClass
      SelectBadConnectionsInNetClass(ComboBoxClasses.Items.GetObject(ComboBoxClasses.ItemIndex));

   if (CheckBoxSelectBadConnections.Checked) and (Board.SelectecObjectCount > 0) then
   begin
        // Added Zoom=True in Fix Connections v1.0
        Client.PostMessage('PCB:RunQuery','Apply=True|Expr=IsSelected|Zoom=True|Mask=True',
            Length('Apply=True|Expr=IsSelected|Mask=True'), Client.CurrentView);
   end;

   If (Board.SelectecObjectCount = 0) then
   begin
        // Added in Fix Connections v1.0 (PCB view if no bad connections are found)
        Client.PostMessage('PCB:RunQuery','Apply=True|Expr=Not(IsSelected)|Zoom=False|Mask=False',
            Length('Apply=True|Expr=Not(IsSelected)|Mask=False'), Client.CurrentView);
   end;

   PCBServer.SystemOptions.DoOnlineDRC := OnLineDRC;

   // Code Elapsed Time added in Fix Copnnections v1.0
   delta := GetTickCount - startTick;
   lblTime.Caption:= 'Elapsed Time : ' + IntToStr(delta) + ' mSec';  // Update Form
   Showmessage ('Found Bad Connections : Qty = ' +  IntToStr(cnt) + #13#10 );

   close;
end;

procedure TFixConnectionsForm.EditToleranceChange(Sender: TObject);
begin
...
procedure TFixConnectionsForm.CheckBoxFixOverlapsClick(Sender: TObject);
begin
   CheckBoxSplitTTracks.Enabled := CheckBoxFixOverlaps.Checked;
   CheckBoxSplitTTracks.Checked := CheckBoxFixOverlaps.Checked;
end;

procedure TFixConnectionsForm.CheckBoxSelectBadConnectionsClick(Sender: TObject);
begin
